...
# 240227 - new heatmaps for species pairs
# 240222 - making new heatmaps for orthogroup gene composition 
#        - also need to correct for some errors in reading the files
# 240214 - need to make new heatmaps for orthogroup gene composition
...
#############################################################
metrics_dip_path <- paste0(work_dir, "comparison/240212_diploid/diploid_newMetrics_noDup_240221.csv")
metrics_mix_path <- paste0(work_dir, "comparison/240209_full/full_newMetrics_noDup_240221.csv")
...
metrics_dip %>%
  ...
  summarise(
    ...
    mean_se_high = mean + se,
    mean_se_low = mean - se,
    .groups = 'keep') -> metrics_dip_R2

metrics_dip %>%
  group_by(program1, program2) %>%
  filter(!grepl(NaN, ARS)) %>%
...
metrics_dip %>%
  ...
  summarise(
    n = n(),
    mean = mean(ARS, na.rm = TRUE),
    median = median(ARS, na.rm = TRUE),
    sd   = sd(ARS, na.rm = TRUE),
    se = sd/sqrt(n),
    mean_se_high = mean + se,
    mean_se_low = mean - se,
    .groups = 'keep') -> metrics_dip_A2

metrics_dip %>%
  group_by(program1, program2) %>%
  filter(!grepl(NaN, Jaccard)) %>%
...
# Export the file
metrics_out <- paste0(work_dir,"comparison/240214_combine/metricsSummary_DipMix_240222.xlsx")
...
## print plots ##
plots_metrics_dip <- paste0(work_dir, "comparison/240214_combine/metricsSummaryDiploid_240222.pdf")
...
plots_metrics_mix <- paste0(work_dir, "comparison/240214_combine/metricsSummaryMixed_240222.pdf")
...
####################################################################
spPair_dip_path <- paste0(work_dir, "comparison/240223_pairs/spPairs_diploid_JIsummary_240223.csv")
spPair_mix_path <- paste0(work_dir, "comparison/240223_pairs/spPairs_mixed_JIsummary_240227.csv")

spPair_dip <- read.csv(spPair_dip_path)
spPair_mix <- read.csv(spPair_mix_path)

## heatmap - diploid ##
spPair_dip$method <- factor(spPair_dip$algorithm,
                            levels = c("BR", "OFb", "OFd", "OFm", "SPd", "SPm", "ON"))
sp1 <- ggplot(spPair_dip, aes(x=algorithm, y=SpeciesPair, fill= proportionOfExactOG)) + 
  geom_tile() +
...
sp1_1 <- ggplot(spPair_dip, aes(x=algorithm, y=SpeciesPair, fill= proportionOfExactOG)) + 
  geom_tile() +
  scale_fill_distiller(palette = "Blues", trans = "reverse", limits = c(1, 0)) +
...
sp2 <- ggplot(spPair_dip, aes(x=algorithm, y=SpeciesPair, fill= meanJI)) + 
  geom_tile() +
...
## heatmap - mixed ##
spPair_mix$method <- factor(spPair_mix$algorithm,
                            levels = c("BR", "OFb", "OFd", "OFm", "SPd", "SPm", "ON"))
sp3 <- ggplot(spPair_mix, aes(x=algorithm, y=SpeciesPair, fill= proportionOfExactOG)) + 
  geom_tile() +
  scale_fill_distiller(palette = "Blues", trans = "reverse", limits = c(1, 0)) +
  geom_text(aes(label = numberOfExactOG)) + 
...
sp4 <- ggplot(spPair_mix, aes(x=algorithm, y=SpeciesPair, fill= proportionOfExactOG)) + 
  geom_tile() +
  scale_fill_distiller(palette = "Blues", trans = "reverse", limits = c(1, 0)) +
  geom_text(aes(label = round(proportionOfExactOG, digits=3))) + 
...
sp5 <- ggplot(spPair_mix, aes(x=algorithm, y=SpeciesPair, fill= meanJI)) + 
  geom_tile() +
  scale_fill_distiller(palette = "Greens", trans = "reverse", limits = c(1, 0)) +
  geom_text(aes(label = round(meanJI,digits=3))) + 
...
plots_speciesPair_dip <- paste0(work_dir, "comparison/240223_pairs/heatmap_SpeciesPairs_Diploid_240227.pdf")
pdf(plots_speciesPair_dip)
print(sp1)
print(sp1_1)
print(sp2)
dev.off()
plots_speciesPair_mix <- paste0(work_dir, "comparison/240223_pairs/heatmap_SpeciesPairs_Mixed_240227.pdf")
