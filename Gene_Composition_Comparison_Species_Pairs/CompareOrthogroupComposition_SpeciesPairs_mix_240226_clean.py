...
def masterSpeciesPairs(filePath, OGprogram, columnOG):
    ...
    for item in pairs:
        ...
        if species1 == "Ath":
            ...
            if species2 == "Aar":
            	JI_DF = calcJaccardList(mergeDF, columnOG, "Aa", spPair)
            	speciesPairMasterList.append(JI_DF)
            else: 
            	JI_DF = calcJaccardList(mergeDF, columnOG, "Sal", spPair)
            	speciesPairMasterList.append(JI_DF)
        elif species1 == "Bra":
            if species2 == "Aar":
            	JI_DF = calcJaccardList(mergeDF, columnOG, "Aa", spPair)
            	...
            	JI_DF = calcJaccardList(mergeDF, columnOG, "Bra", spPair)
            	speciesPairMasterList.append(JI_DF)
        elif species1 == "Csa" and species2 == "Aar": # use Aar gene as the "reference because it is a diploid
            JI_DF = calcJaccardList(mergeDF, columnOG, "Aa", spPair)
            speciesPairMasterList.append(JI_DF)
...
summary_path = "/path_to_output/mixed_speciesPairsJIvalues_toCorrect_240226.csv"
methods_concat_nodup.to_csv(summary_path, sep = ",", index="False")  
