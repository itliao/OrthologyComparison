...
def reformattedOG_atleast1(AT_ref_gene, OG_cluster):
    ...
    for gene2 in geneList:
        if gene2.startswith("Aa"):
            AT_new_OG[AT_ref_gene]['Aar'].append(gene2)
        elif gene2.startswith("AT"):
            AT_new_OG[AT_ref_gene]['Ath'].append(gene2)
        elif gene2.startswith("Bra"):
            AT_new_OG[AT_ref_gene]['Bra'].append(gene2)
        elif gene2.startswith("CAR"):
            AT_new_OG[AT_ref_gene]['Chi'].append(gene2)
        elif gene2.startswith("Car"):
            AT_new_OG[AT_ref_gene]['Cru'].append(gene2)
        elif gene2.startswith("Csa"):
            AT_new_OG[AT_ref_gene]['Csa'].append(gene2)
        elif gene2.startswith("Sal"):
            AT_new_OG[AT_ref_gene]['Sal'].append(gene2)
...
def reformattedOG_common(AT_ref_gene, OG_cluster, atRow):
    ...
    for index, row in OG_cluster.iterrows():
        gene2 = row[0]
        if gene2.startswith("Aa"):
            Aar_match = countMatchingCluster(atRow, row)
            if Aar_match >= 5:
                AT_new_OG[AT_ref_gene]['Aar'].append(gene2)
        elif gene2.startswith("AT"):
            ...
            Bra_match = countMatchingCluster(atRow, row)
            if Bra_match >= 5:
                AT_new_OG[AT_ref_gene]['Bra'].append(gene2)
        elif gene2.startswith("CAR"):
            ...
            Csa_match = countMatchingCluster(atRow, row)
            if Csa_match >= 5:
                AT_new_OG[AT_ref_gene]['Csa'].append(gene2)
        elif gene2.startswith("Sal"):
            Sal_match = countMatchingCluster(atRow, row)
            if Sal_match >= 5:
                AT_new_OG[AT_ref_gene]['Sal'].append(gene2)
        elif gene2.startswith("gene"):
...
metricsList=[]
merge6["SPd"] = merge6["SPd"].astype("str")
merge6["SPm"] = merge6["SPm"].astype("str")
    ...
    if gene.startswith("AT"):
        ...
        for i in range(len(programList)-1):
            for j in range(i+1,7):
                ...
                # subset dataframe for each program; find all members in the same cluster
                ...
                metricsList.append([gene,p1,p2,RS,ARS,intersect,un,jaccard])
                print(RS, ARS, intersect, un, jaccard) 

####################################### 
# step 3: convert list into dataframe #
#######################################
metricColumns = ["gene", "program1", "program2", "RS","ARS","intersection", "union","Jaccard"]
metricsDF = pd.DataFrame(metricsList, columns = metricColumns)
          
metrics_out = main_path + "/output/full_pairwiseMetrics_231004.csv"
